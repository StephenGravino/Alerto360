    ...
    if ($name && $email && $password) {
        ...
        if ($check->fetch()) {
            ...
            if (!in_array($responder_type, ['PNP', 'BFP', 'MDDRMO'])) {
                ...
                // Ensure responder_type column exists (run this only once, safe to ignore error if already exists)
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-4">
            <div class="card p-4 h-100">
                <h5 class="section-title mb-3">Add Responder Account</h5>
                <?= $add_responder_msg ?>
                <form method="post" class="row g-2">
                    <input type="hidden" name="add_responder" value="1">
                    <div class="col-12 mb-2"><input type="text" class="form-control" name="name" placeholder="Name" required></div>
                    ...
                    <div class="col-12"><button type="submit" class="btn btn-primary w-100">Add Responder</button></div>
                </form>
            </div>
        </div>
        <!-- Responder Accounts -->
        <div class="col-lg-8">
            <div class="card p-4 h-100">
                <h5 class="section-title mb-3">Responder Accounts</h5>
    ...
    <?php endif; ?>
                        ...
                        <td>
                            <?php if (isset($_GET['edit']) && $_GET['edit'] == $responder['id']): ?>
                                ...
                                <a href="admin_dashboard.php" class="btn btn-secondary btn-sm">Cancel</a>
                                </form>
                            <?php else: ?>
                                <form method="post" style="display:inline;">
                                ...
                                <a href="admin_dashboard.php?edit=<?= $responder['id'] ?>" class="btn btn-warning btn-sm">Edit</a>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
